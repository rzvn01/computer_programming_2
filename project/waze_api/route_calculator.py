...
class RouteCalculator():
    ...
    def calc_all_routes_info(self, paths_number=3, real_time=True, stop_at_bounds=False, delta_time=0):

        routes = self.get_route(paths_number, delta_time)
        try:
            results = {"%s-%s" % (
                ''.join(route.get('routeType', [])[:1]), route.get('shortRouteName', 'unkown')): self._add_up_route(
                route['results' if 'results' in route else 'result'], real_time=real_time,
                stop_at_bounds=stop_at_bounds) for route in routes}
        except KeyError:
            raise RouteError("wrong response")

        route_time = [route[0] for route in results.values()]
        route_distance = [route[1] for route in results.values()]
        print('Min\tMax\n%.2f\t%.2f minutes\n%.2f\t%.2f km', min(route_time), max(route_time),
